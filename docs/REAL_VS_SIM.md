...
| Insurance pool (fund / claim / withdraw) | ✅ REAL | Insurance Pool contract. Fund flow: user → escrow (deducts `insurance_fee_bps` = 200 bps = 2%) → pool. Claim replay guarded by a global `claimed_escrow_ids` tombstone (see `docs/INSURANCE_REPLAY_TESTS.md`). **Caveat (found 2026-07-28, see `DEPLOYMENT_LESSONS.md`):** `GET /insurance/pool-stats`'s `total_assets`/`total_claims_paid`/etc. are a separate **in-memory** counter (`server/insurance.py: _insurance_pool`), not read from the real on-chain purse — they reset on every backend redeploy and do not reconcile with the actual purse balance. `deposit`/`claim` still execute real on-chain transfers regardless (verified via `query_global_state` balance deltas); only the *dashboard numbers* are off-chain bookkeeping. |
