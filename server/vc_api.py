...
def get_issuer_key() -> IssuerKey:
    ...
    if seed is None:
        raise HTTPException(
            status_code=503,
            detail=(
                "VC_ISSUER_SEED is not configured or not a 32-byte seed " "(accepted: base64/base64url/hex/raw ASCII)."
            ),
        )
...
class IssueRequest(BaseModel):
    ...
    asset: str = "CSPR"
    issuance_ts: int | None = Field(None, description="Unix epoch seconds; defaults to now")
...
class VerifyRequest(BaseModel):
    credential: dict[str, Any]
    expected_issuer: str | None = Field(None, description="Optional did:key to enforce")
...
async def verify(req: VerifyRequest) -> VerifyResponse:
    ...
    try:
        ...
        return VerifyResponse(valid=False, error_type="SignatureInvalid", error_detail=str(exc))
    except ProofMissingError as exc:
        return VerifyResponse(valid=False, error_type="ProofMissing", error_detail=str(exc))
    except SchemaError as exc:
        return VerifyResponse(valid=False, error_type="Schema", error_detail=str(exc))
    except VerificationError as exc:
        return VerifyResponse(valid=False, error_type="Verification", error_detail=str(exc))

    return VerifyResponse(valid=True, summary=receipt_summary(req.credential))
...
def try_auto_issue(
    *,
    ...
    seed = _decode_seed(raw)
    if seed is None:
        logger.warning("VC_AUTO_ISSUE_ON_RELEASE=1 but VC_ISSUER_SEED not configured; skipping receipt")
        return None
    try:
