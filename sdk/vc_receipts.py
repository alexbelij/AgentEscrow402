...
def _jcs_canonicalize(value: Any) -> bytes:
    ...
    def _encode(v: Any) -> str:
        ...
        if isinstance(v, float):
            raise TypeError(
                "JCS: float values not permitted in AE402 receipts "
                "(use integer motes)"
            )
        if isinstance(v, str):
        ...
        if isinstance(v, dict):
            ...
            return (
                "{"
                + ",".join(f"{json.dumps(k, ensure_ascii=False)}:{_encode(val)}" for k, val in items)
                + "}"
            )
    ...
    *,
    ...
    if expected_issuer is not None and issuer_did != expected_issuer:
        raise VerificationError(
            f"issuer mismatch: expected {expected_issuer!r}, got {issuer_did!r}"
        )
