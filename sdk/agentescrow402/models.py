...
class AgentIdentity(BaseModel):
    ...
    capabilities: List[str] = Field(default_factory=list, description="List of capabilities/skills the agent possesses.")
    reputation_score: Optional[float] = Field(None, ge=0, le=100, description="Reputation score based on past performance.")
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Timestamp when the agent identity was registered.")
    updated_at: datetime = Field(default_factory=datetime.utcnow, description="Timestamp of the last update to the agent identity.")
...
class StreamConfig(BaseModel):
    ...
    total_periods: int = Field(..., gt=0, description="Total number of periods over which the payment will be streamed.")
    start_time: Optional[datetime] = Field(None, description="Optional start time for streaming. Defaults to immediate if not provided.")
...
class EscrowResponse(BaseModel):
    ...
    deadline: Optional[datetime] = Field(None, description="Optional deadline for the escrow service completion.")
    streaming_config: Optional[StreamConfig] = Field(None, description="Configuration for streaming payments, if applicable.")
...
class EscrowActionRequest(BaseModel):
    ...
    amount: Optional[float] = Field(None, gt=0, description="Amount for 'fund', 'release', 'refund' actions. Optional for full release/refund.")
