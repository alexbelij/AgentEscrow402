...
| 2 | **Batch Escrow Manager (v11)** | `1b93d536947da31ed80d6b57a5db74c718b6cf08f33e5b0bdd893d27f481dd0c` | `c423a07f334ae4c5badf7fcfe6c595abc1d7ba07fdfc43a0464525aa416fe4d6` | 11 | [live](https://testnet.cspr.live/contract/c423a07f334ae4c5badf7fcfe6c595abc1d7ba07fdfc43a0464525aa416fe4d6) |
...
`deploy-out/onchain.json` is **hand-maintained and verified**, not generated by a
single script — there is no `deploy_all_contracts.py` or `emit_onchain_manifest.py`
in this repo. Each contract is deployed individually (per-contract `.mjs` scripts
in `server/casper_tx/`, or the raw `casper-client put-deploy` recipes in
`docs/ONCHAIN_DEPLOY.md`), and the resulting `contract_hash`/`deploy_hash` is
copied into `deploy-out/onchain.json` by hand. After any redeploy or new
contract:

```bash
python3 scripts/audit_contract_artifact.py <target> --strict   # verify the entry
                                                                 # you just wrote
                                                                 # matches on-chain state
python3 scripts/generate_frontend_manifest.py                  # regenerate the
                                                                 # frontend's typed
                                                                 # manifest.generated.ts
                                                                 # from onchain.json
```

`tests/test_manifest_frontend_config_sync.py` enforces `manifest.generated.ts`
is never stale relative to `onchain.json`. Then update the tables above and
bump the "Last regenerated" date.
