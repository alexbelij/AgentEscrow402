...
STALE_HASHES=(
  "e128780f"   # old insurance-pool (superseded 2026-07-18 by ead90738)
  "e36b958d"   # even-older insurance-pool (fully public claim/withdraw)
  "5d65bedf"   # old vrf-arbiter (Mint error 21 on register_arbiter)
)
...
for hash in "${STALE_HASHES[@]}"; do
  # Collect files that contain the hash, excluding vendored / built dirs and
  # generated caches. -I skips binary files (pyc, wasm, etc.).
  mapfile -t hits < <(
    grep -rln -I "$hash" \
  ...
  for file in "${hits[@]}"; do
    ...
    # Per-line context check: every line mentioning the hash must ALSO contain
    # a marker word from CONTEXT_MARKERS.
...
if [ "$FAIL" -eq 1 ]; then
  ...
  echo "  * Replacing the hash with the current one from server/config.py, OR"
