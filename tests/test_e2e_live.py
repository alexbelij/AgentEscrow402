...
check("compute-hash.service_hash is hex64", isinstance(body, dict) and len(body.get("service_hash", "")) == 64)
...
check(
    "vrf.elected_arbiter present",
    isinstance(body, dict) and "elected_arbiter" in body,
    str(list(body.keys()) if isinstance(body, dict) else body),
)
check("vrf.dispute_id matches", isinstance(body, dict) and body.get("dispute_id") == vrf_payload["dispute_id"])
check("vrf.election_proof present", isinstance(body, dict) and bool(body.get("election_proof")))
...
check("POST /arbitration/analyze → 200 or 401", code in (200, 201, 401, 422), f"got {code} body={str(body)[:200]}")
if code in (200, 201) and isinstance(body, dict):
    check(
        "arbitration.recommendation present", "recommendation" in body or "dispute_id" in body, str(list(body.keys()))
    )
